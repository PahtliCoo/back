...
public class GetAllRequestByUserIdAndStateUseCase {
    ...
    public List<Request> execute(int userId, GetRequestByStateReqDTO getRequestByStateReqDTO) {
        try{
            // 1. get all the request requests that have a state and
            List<Request> requestList = requestService.getAllRequestsByUserIdByState(userId, getRequestByStateReqDTO.getState());
            return requestList;
        }catch (Exception e) {
